...
(function() {
  $(document).on("ajax:beforeSend", "#create_comment_form", function(evt, xhr, settings) {
    return $(this).find('textarea').addClass('uneditable-input').attr('disabled', 'disabled');
  }).on("ajax:success", "#create_comment_form", function(evt, data, status, xhr) {
    $(this).find('textarea').removeClass('uneditable-input').removeAttr('disabled', 'disabled').val('');
    return $('<div class="">').html(xhr.responseText).hide().insertAfter($(this)).show('slow');
  });
